...
?>
<p class="mb-20">Sebuah email berisi kode validasi OTP telah dikirimkan ke email anda. Silahkan masukkan kode tersebut disini</p>
<p class="mb-20">(We have send OTP Code to your email. Please input the code here)</p>
<?php
...
<input type="hidden" name="tipe_layanan" value="<?=$this->input->post('tipe_layanan')?>">
  ...
  <div class="form-group row">
    <label for="staticEmail" class="col-sm-4 col-form-label">Kode OTP<br>OTP Code</label>
    <div class="col-sm-8">
      <input type="text" name="otp" class="form-control" placeholder="">
    </div>
  </div>

<button type="submit" class="main-btn">Send OTP  <i class="fal fa-arrow-right"></i></button>
                        </div>
<?php
	echo form_close();
?>
<br/><br/><br/>
<?php
	echo form_open('user/registerotp', 'class="form-horizontal"');
?>
<input type="hidden" name="nama_depan" value="<?=$this->input->post('nama_depan')?>">
<input type="hidden" name="nama_tengah" value="<?=$this->input->post('nama_tengah')?>">
<input type="hidden" name="nama_belakang" value="<?=$this->input->post('nama_belakang')?>">
...
function countdown( elementName, minutes, seconds )
{
    ...
    function updateTimer()
    {
        ...
        if ( msLeft < 1000 ) {
            element.innerHTML = "Time is up! Silahkan Kirim Ulang OTP <button type=\"submit\" class=\"main-btn\">Resend OTP  <i class=\"fal fa-arrow-right\"></i></button>";
        } else {
            time = new Date( msLeft );
            hours = time.getUTCHours();
            mins = time.getUTCMinutes();
            element.innerHTML = 'Anda dapat mengirim ulang kode opt dalam waktu : ' + (hours ? hours + ':' + twoDigits( mins ) : mins) + ':' + twoDigits( time.getUTCSeconds() );
