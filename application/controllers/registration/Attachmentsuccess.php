...
class Attachmentsuccess extends Baseregistration {
	...
	public function index()
	{
		...
		$data = array(	'title'	=>	$config->site_title,
				...
				'progress' => 84,
				'config' => $config,
				'isi'	=>	'home/'.$config->template.'/registration/13-attachmentsuccess'
			);
	...
	public function next(){
		$result = [];
		if(isset($_SESSION['step']) && $_SESSION['step'] == 'attachmentsuccess'){
			...
			try{
				if($this->input->server('REQUEST_METHOD') === 'POST' 
					&& $this->session->has_userdata('formid')){
		      		...
		      		$result['response'] = '1';
		      		$result['next-page'] = base_url('registration/review');
		      		$this->setSessionStep('review');
				} else{
					$result['response'] = '0';
