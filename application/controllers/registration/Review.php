...
class Review extends Baseregistration {
	...
	public function index()
	{
		...
		$data = array(	'title'	=>	$config->site_title,
				...
				'progress' => 90,
				'config' => $config,
				'isi'	=>	'home/'.$config->template.'/registration/14-review'
			);
	...
	public function next(){
		$result = [];
		if(isset($_SESSION['step']) && $_SESSION['step'] == 'review'){
			...
			try{
				if($this->input->server('REQUEST_METHOD') === 'POST' 
		            ...
		            $result['response'] = '1';
		      		$result['next-page'] = base_url('registration/syaratketentuan');
		      		$this->setSessionStep('syaratketentuan');
