...
class Attachmentguide extends Baseregistration {
	...
	public function index()
	{
		...
		$data = array(	'title'	=>	$config->site_title,
				...
				'progress' => 66,
				'config' => $config,
				'isi'	=>	'home/'.$config->template.'/registration/10-attachmentguide'
			);
	...
	public function next(){
		$result = [];
		if(isset($_SESSION['step']) && $_SESSION['step'] == 'attachmentguide'){
			...
			try{
				if($this->input->server('REQUEST_METHOD') === 'POST' 
					&& $this->session->has_userdata('formid')){
		      		...
		      		$result['response'] = '1';
		      		$result['next-page'] = base_url('registration/selfiektp');
		      		$this->setSessionStep('selfiektp');
				} else{
					$result['response'] = '0';
