...
//		$menu = $this->menu_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Top Menu',
						'config'	=> $config,
						'menu' => $menu,
						'isi'	=>	'admin/menu/list');
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Top Menu',
							'menu'	=>	$menu,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
...
//							'userid'	=> $this->session->userdata('userid'),
	...
	public function edit($id_menu)
	{
			...
			$menulist = $this->menu_model->listing();
//			$sized = $this->sized_model->listing();
//			$colors = $this->colors_model->listing();
		$config = $this->config_model->detail(1);
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Top Menu',
							'menu'	=> $menu,
							'menulist'	=>	$menulist,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/menu/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/menu'));// redirect to show file type not support message
  }
}
