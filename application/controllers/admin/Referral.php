...
class Referral extends CI_Controller {
	...
	{
		$referral = $this->referral_model->listing();
//		$referral = $this->referral_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Data Referral',
						'config'	=> $config,
						'referral' => $referral,
						'isi'	=>	'admin/referral/list');
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Data Referral',
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/referral/add');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//							'url'	=> $url
	...
	public function edit($id_referral)
	{
			$referral = $this->referral_model->detail($id_referral);
//			$category = $this->category_model->listing();
//			$sized = $this->sized_model->listing();
//			$colors = $this->colors_model->listing();
		$config = $this->config_model->detail(1);
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Data Referral',
							'referral'	=> $referral,
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/referral/edit');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//exit();
	...
	public function delete($id_referral)
	{
		$referral = $this->referral_model->detail($id_referral);
		$data = array('id' => $id_referral);
		$thefile = $referral->image;
//		echo $thefile;
		unlink($thefile);
//	exit;
		$this->referral_model->delete($data);
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/referral/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/referral'));// redirect to show file type not support message
  }
}
