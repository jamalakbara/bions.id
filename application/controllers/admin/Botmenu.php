...
class Botmenu extends CI_Controller {
...
//		$botmenu = $this->botmenu_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Footer Menu',
						'config'	=> $config,
						'botmenu' => $botmenu,
						'isi'	=>	'admin/botmenu/list');
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Footer Menu',
							'botmenu'	=>	$botmenu,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/botmenu/add');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//							'userid'	=> $this->session->userdata('userid'),
			...
			$this->botmenu_model->add($data);
			$this->session->set_flashdata('sukses','Data has been add');
			redirect(base_url('admin/botmenu'));
		}

	}

	public function edit($id_botmenu)
	{
			$botmenu = $this->botmenu_model->detail($id_botmenu);
			$botmenulist = $this->botmenu_model->listing();
//			$sized = $this->sized_model->listing();
//			$colors = $this->colors_model->listing();
		$config = $this->config_model->detail(1);
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Footer Menu',
							'botmenu'	=> $botmenu,
							'botmenulist'	=>	$botmenulist,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/botmenu/edit');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//echo $this->input->post('newtab'); exit;

			$data = array(	'id'		=> $id_botmenu,
							'title'		=> $i->post('title'),
							'newtab'	=> $this->input->post('newtab'),
...
//exit();
	...
	public function delete($id_botmenu)
	{
		$botmenu = $this->botmenu_model->detail($id_botmenu);
		$data = array('id' => $id_botmenu);
		$thefile = $botmenu->image;
//		echo $thefile;
		unlink($thefile);
//	exit;
		$this->botmenu_model->delete($data);
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/botmenu/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/botmenu'));// redirect to show file type not support message
  }
}
