...
class Pekerjaan extends CI_Controller {
	...
	{
		$pekerjaan = $this->tabel_model->listing('pekerjaan');
...
//		$valid->set_rules ('stock','Stock','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Data Pekerjaan',
						'config'	=> $config,
							'isi'	=>	'admin/pekerjaan/add');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//						);
			$data = array(	'pekerjaan'	=> $this->input->post('pekerjaan'),
	...
	{
			$pekerjaan = $this->tabel_model->detail('pekerjaan',$id_pekerjaan);
		$config = $this->config_model->detail(1);
