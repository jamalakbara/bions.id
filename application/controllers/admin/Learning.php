...
class Learning extends CI_Controller {
	...
	public function index()
	{
		...
		if($catid) {
			$learning = $this->learning_model->listingcat($catid);
			$cattitle = $this->categories_model->detail($catid);
		} else {
			$learning = $this->learning_model->listing();
			$cattitle = '';
		}
//		$learning = $this->learning_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Data Learning '.$cattitle->title,
						'catid'	=> $catid,
						'config'	=> $config,
						'learning' => $learning,
						'isi'	=>	'admin/learning/list');
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Data Learning '.$cattitle->title,
							'catid'	=> $catid,
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
							'config'	=> $config,
							'isi'	=>	'admin/learning/add');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;

			$file_path = realpath(APPPATH.'../images/learning');
                $configu['upload_path']          = $file_path;
                $configu['allowed_types']        = 'gif|jpg|jpeg|png';
//                $configu['allowed_types']        = 'gif|jpg|png|pdf|mp4|mp3|ogg';
//                $configu['allowed_types']        = 'pdf|mp4';
//                $configu['max_size']             = 30720;
//                $configu['max_width']            = 4288;
//                $configu['max_height']           = 2848;
//echo $file_path;
...
//							'userid'	=> $this->session->userdata('userid'),
			...
			$this->learning_model->add($datadb);
			$this->session->set_flashdata('sukses','Data berhasil ditambah');
			redirect(base_url('admin/learning?catid='.$catid));
				}
		}

	}

	public function edit($id_learning)
	{
		$catid = $this->input->get('catid');
		if($catid) {
			$cattitle = $this->categories_model->detail($catid);
		...
		$learning = $this->learning_model->detail($id_learning);
		$config = $this->config_model->detail(1);
		...
		if($valid->run() === FALSE) {
			...
			if(basename($_FILES['fileinput']['name']) == "") { 
				$datafile = array(	'file_name'	=> $learning->file_name,
									'file_type'	=> $learning->file_type,
									'orig_name'	=> $learning->orig_name,
									'client_name' => $learning->client_name,
									'file_ext'	=>	$learning->file_ext);
			} else { 
...
//				echo print_r($data); exit();
				$this->learning_model->edit($datadb);
	...
	public function edits($id_learning)
	{
			$learning = $this->learning_model->detail($id_learning);
//			$category = $this->category_model->listing();
//			$sized = $this->sized_model->listing();
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Data Learning',
							'learning'	=> $learning,
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
...
//echo $namafile2;
//exit();
			$this->learning_model->edit($data);
			$this->session->set_flashdata('sukses','Data has been edit');
			redirect(base_url('admin/learning'));
		}

	}

	public function delete($id_learning)
	{
//		$learning = $this->learning_model->detail($id_learning);
		$data = array('id' => $id_learning);
//		$thefile = $learning->image;
//		echo $thefile;
//		unlink($thefile);
//	exit;
		$catid = $this->input->get('catid');
		...
		$this->learning_model->delete($data);
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/learning/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/learning'));// redirect to show file type not support message
  }
}

	function get_LearningDatasByUrl(){
		// POST data
		$postData = $this->input->post("meta_slug");

		$learningbyurl = $this->learning_model->get_LearningDatasByUrl($postData);

		echo json_encode($learningbyurl);
	}
