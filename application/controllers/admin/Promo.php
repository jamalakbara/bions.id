...
class Promo extends CI_Controller {
	...
	{
		$promo = $this->promo_model->listing();
//		$promo = $this->promo_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Data Promo',
						'config'	=> $config,
						'promo' => $promo,
						'isi'	=>	'admin/promo/list');
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Data Promo',
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
...
//							'url'	=> $url
	...
	{
			$promo = $this->promo_model->detail($id_promo);
//			$category = $this->category_model->listing();
//			$sized = $this->sized_model->listing();
//			$colors = $this->colors_model->listing();
		$config = $this->config_model->detail(1);
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Data Promo',
							'promo'	=> $promo,
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/promo/edit');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/promo/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/promo'));// redirect to show file type not support message
  }
}
