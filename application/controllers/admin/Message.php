...
class Message extends CI_Controller {
    ...
    public function __construct()
    {
         ...
         $this->load->model('messagemaster_model');
	...
	{
		$messagemaster = $this->messagemaster_model->listing();
		$message = $this->message_model->listing();
//		$message = $this->message_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Data Message',
						'config'	=> $config,
						'message' => $message,
	...
	public function listing($botid)
	{
		$messagemaster = $this->messagemaster_model->detail($botid);
		$message = $this->message_model->listbotid($messagemaster->botid);
//		$message = $this->message_model->listingbyuser($this->session->userdata('userid'));
		$config = $this->config_model->detail(1);

		$data = array(	'title'	=>	'Data Message '.$messagemaster->botid.' - '.$messagemaster->nama.', '.$messagemaster->email.', '.$messagemaster->handphone,
						'config'	=> $config,
						'message' => $message,
						'messagemaster' => $messagemaster,
						'isi'	=>	'admin/message/listbotid');
		$this->load->view('admin/layout/wrapper',$data);
	}
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Data Message',
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/message/add');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//							'url'	=> $url
	...
	{
			$message = $this->message_model->detail($id_message);
//			$category = $this->category_model->listing();
//			$sized = $this->sized_model->listing();
//			$colors = $this->colors_model->listing();
		$config = $this->config_model->detail(1);
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Data Message',
							'message'	=> $message,
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/message/edit');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//exit();
	...
	{
		$messagemaster = $this->messagemaster_model->detail($id_message);
		$data = array('id' => $id_message);
//		$thefile = $message->image;
//		echo $thefile;
//		unlink($thefile);
//	exit;
		$this->messagemaster_model->delete($data);
		
		$message = $this->message_model->listbotid($messagemaster->botid);
//		foreach($message as $row) {
			$data1 = array('botid' => $messagemaster->botid);
			$this->message_model->deletebotid($data1);
//		}
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/message/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/message'));// redirect to show file type not support message
  }
}
