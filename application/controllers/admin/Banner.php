...
//		$banner = $this->banner_model->listingbyuser($this->session->userdata('userid'));
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Tambah Data Banner',
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
						'config'	=> $config,
							'isi'	=>	'admin/banner/add');
			$this->load->view('admin/layout/wrapper',$data);
		} else {
			$i = $this->input;
...
//		$valid->set_rules ('category','Category','required');
		...
		if($valid->run() === FALSE) {
			$data = array(	'title'	=>	'Edit Data Banner',
							'banner'	=> $banner,
//							'category'	=>	$category,
//							'colors'	=>	$colors,
//							'sized'	=>	$sized,
...
function do_upload($inputfie)
{
  ...
  {
    $tmppath="images/banner/".$this->session->userdata('userid').$tnow.uniqid(rand()).".".$type; // uniqid(rand()) function generates unique random number.
  ...
  {	
    redirect(base_url('admin/banner'));// redirect to show file type not support message
  }
}
