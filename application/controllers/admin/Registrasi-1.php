...
//	exit;
	...
	public function exportxls()
	{
        ...
        if(count($ambildata)>0){
            ...
            $val = array("FirstName","MiddleName","LastName","BirthPlace","BirthDate","Sex","ID.No","ID Place","ID Date","ID ExpDate","Religion","Education","MotherName","MaritalStatus","SpouseName","MobilePhone","email","ktp adr1","ktp adr2","ktp village","ktp subdictrict","ktp city","ktp prov","ktp zipcode","CurrentAdr","HomeTelp(2)","HomeTelp(3)","bankid","bankaccname","bankaccno","npwp","compname","comp adr1","comp adr2","comp village","comp subdistrict","comp zipcode","cur .adr1","cur .adr2","cur .village","cur .subdictrict","cur .city","cur .prov","cur .cty","cur .zipcode");
...
//exit();
            ...
            foreach ($ambildata as $frow){
                ...
                $objset->setCellValue("A".$baris, $frow->nama_depan); 
                $objset->setCellValue("B".$baris, $frow->nama_tengah); 
                $objset->setCellValue("C".$baris, $frow->nama_depan); 
                $objset->setCellValue("D".$baris, $frow->tempat_lahir); 
                $objset->setCellValue("E".$baris, date('d-m-Y', strtotime($frow->tanggal_lahir))); 
                $objset->setCellValue("F".$baris, $frow->jenis_kelamin); 
                $objset->setCellValueExplicit("G".$baris, $frow->ktp,PHPExcel_Cell_DataType::TYPE_STRING); 
                $objset->setCellValue("H".$baris, $frow->ktp_tempat); 
                $objset->setCellValue("I".$baris, date('d-m-Y', strtotime($frow->ktp_terbit))); 
                $objset->setCellValue("J".$baris, $ktpberlaku); 
                $objset->setCellValueExplicit("K".$baris, $frow->agama,PHPExcel_Cell_DataType::TYPE_STRING); 
                $objset->setCellValue("L".$baris, $frow->pendidikan); 
                $objset->setCellValue("M".$baris, $frow->ibu_kandung); 
                $objset->setCellValue("N".$baris, $frow->status_pernikahan); 
                $objset->setCellValue("O".$baris, $frow->nama_pasangan); 
                $objset->setCellValue("P".$baris, $frow->handphone); 
                $objset->setCellValue("Q".$baris, $frow->email); 
                $objset->setCellValue("R".$baris, $frow->ktp_alamat); 
                $objset->setCellValue("S".$baris, $frow->ktp_rtrw); 
                $objset->setCellValue("T".$baris, $frow->ktp_kelurahan); 
                $objset->setCellValue("U".$baris, $frow->ktp_kecamatan); 
                $objset->setCellValue("V".$baris, $frow->ktp_cityid); 
                $objset->setCellValue("W".$baris, $frow->ktp_provinceid); 
                $objset->setCellValue("X".$baris, $frow->ktp_kodepos); 
                $objset->setCellValue("Y".$baris, '0'); 
                $objset->setCellValue("Z".$baris, '');
				
                $objset->setCellValue("AA".$baris, $frow->phone); 
                $objset->setCellValue("AB".$baris, $frow->nama_bank); 
                $objset->setCellValue("AC".$baris, $frow->nama_pemilik); 
                $objset->setCellValueExplicit("AD".$baris, $frow->no_rekening,PHPExcel_Cell_DataType::TYPE_STRING); 
                $objset->setCellValueExplicit("AE".$baris, $frow->npwp,PHPExcel_Cell_DataType::TYPE_STRING); 
                $objset->setCellValue("AF".$baris, $frow->nama_perusahaan); 
                $objset->setCellValue("AG".$baris, $frow->alamat_perusahaan); 
                $objset->setCellValue("AH".$baris, $frow->perusahaan_rtrw); 
                $objset->setCellValue("AI".$baris, $frow->perusahaan_kelurahan); 
                $objset->setCellValue("AJ".$baris, $frow->perusahaan_kecamatan); 
                $objset->setCellValue("AK".$baris, $frow->perusahaan_provinceid); 
                $objset->setCellValue("AL".$baris, $frow->perusahaan_kodepos); 
                $objset->setCellValue("AM".$baris, $frow->terkini_rtrw); 
                $objset->setCellValue("AN".$baris, $frow->terkini_kelurahan); 
                $objset->setCellValue("AO".$baris, $frow->terkini_kecamatan); 
                $objset->setCellValue("AP".$baris, $frow->terkini_cityid); 
                $objset->setCellValue("AQ".$baris, $frow->terkini_provinceid); 
                $objset->setCellValue("AR".$baris, $frow->negara); 
                $objset->setCellValue("AS".$baris, $frow->terkini_kodepos); 
