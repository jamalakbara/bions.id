...
?>
	...
	$("#risk3").hide();
//    $("div#inputnpwp").show();
	$("#kode_negara").change(function(){ // Ketika user mengganti atau memilih data provinsi
//		alert('hai');
		getkodearea();
    });
	...
	$("#perusahaan_province").change(function(){ // Ketika user mengganti atau memilih data provinsi
		...
		getperusahaankota();
		getperusahaankodepos();
    });

});

 function getkodearea() {		
		$.ajax({
			type: "POST", // Method pengiriman data bisa dengan GET atau POST
			url: "<?=base_url('user/getkodearea')?>", // Isi dengan url/path file php yang dituju
			data: {cty_id : $("#kode_negara").val()}, // data yang akan dikirim ke file yang dituju
			dataType: "json",
...
//					$("#ktploading").hide(); // Sembunyikan loadingnya
					...
					//document.getElementById("kode_negara").value = response.data_kota;
					$("#kode_area").html(response.data_kota).show();
				}, 10);
			},
 ...
 function getkodenegara() {
		$.ajax({
			type: "POST", // Method pengiriman data bisa dengan GET atau POST
			url: "<?=base_url('user/getkodenegara')?>", // Isi dengan url/path file php yang dituju
			data: {cty_id : $("#ktp_negara").val()}, // data yang akan dikirim ke file yang dituju
			dataType: "json",
			beforeSend: function(e) {
				if(e && e.overrideMimeType) {
					e.overrideMimeType("application/json;charset=UTF-8");
...
//					$("#ktploading").hide(); // Sembunyikan loadingnya
 ...
 function getktpkota() {
		$.ajax({
			type: "POST", // Method pengiriman data bisa dengan GET atau POST
			url: "<?=base_url('user/getkota2')?>", // Isi dengan url/path file php yang dituju
 ...
 function getktpkodepos() {
		$.ajax({
			type: "POST", // Method pengiriman data bisa dengan GET atau POST
			url: "<?=base_url('user/getkodepos')?>", // Isi dengan url/path file php yang dituju
			data: {prov_id : $("#ktp_province").val()}, // data yang akan dikirim ke file yang dituju
			dataType: "json",
			beforeSend: function(e) {
				if(e && e.overrideMimeType) {
...
//					$("#ktploading").hide(); // Sembunyikan loadingnya
					...
					// set isi dari combobox kota
					// lalu munculkan kembali combobox kotanya
					$("#ktp_kodepos").html(response.data_kota).show();
				}, 10);
			},
			error: function (xhr, ajaxOptions, thrownError) { // Ketika ada error
				alert(thrownError); // Munculkan alert error
...
//					$("#ktploading").hide(); // Sembunyikan loadingnya
					...
					// set isi dari combobox kota
					// lalu munculkan kembali combobox kotanya
					$("#terkini_kodepos").html(response.data_kota).show();
				}, 10);
			},
			error: function (xhr, ajaxOptions, thrownError) { // Ketika ada error
				alert(thrownError); // Munculkan alert error
...
//					$("#ktploading").hide(); // Sembunyikan loadingnya
					...
					// set isi dari combobox kota
					// lalu munculkan kembali combobox kotanya
					$("#perusahaan_kodepos").html(response.data_kota).show();
				}, 10);
			},
			error: function (xhr, ajaxOptions, thrownError) { // Ketika ada error
				alert(thrownError); // Munculkan alert error
...
numb = numb.toFixed(2);
if(numb > 2){
alert('Foto KTP is to big, maximum is 2MiB. You file size is: ' + numb +' MiB');
document.getElementById("fotoktp").value = '';
//} else {
//alert('it okey, your file has ' + numb + 'MiB')
}
});

$('#fotoselfie').on('change', function() {
 var numb = $(this)[0].files[0].size/1024/1024;
numb = numb.toFixed(2);
if(numb > 2){
alert('Foto Selfie is to big, maximum is 2MiB. You file size is: ' + numb +' MiB');
document.getElementById("fotoselfie").value = '';
}
});

$('#fotopaspor').on('change', function() {
 var numb = $(this)[0].files[0].size/1024/1024;
numb = numb.toFixed(2);
if(numb > 2){
alert('Foto Paspor is to big, maximum is 2MiB. You file size is: ' + numb +' MiB');
document.getElementById("fotopaspor").value = '';
}
});

$('#fotokitas').on('change', function() {
 var numb = $(this)[0].files[0].size/1024/1024;
numb = numb.toFixed(2);
if(numb > 2){
alert('Foto KITAS is to big, maximum is 2MiB. You file size is: ' + numb +' MiB');
document.getElementById("fotokitas").value = '';
}
});

$('#fototabungan').on('change', function() {
 var numb = $(this)[0].files[0].size/1024/1024;
numb = numb.toFixed(2);
if(numb > 2){
alert('Foto Buku Tabungan is to big, maximum is 2MiB. You file size is: ' + numb +' MiB');
document.getElementById("fototabungan").value = '';
}
});
...
chatBtn.addEventListener('click', ()=>{
    popup.classList.toggle('show');
		 ...
		 function send_msg(){
			...
			if(txt){
				jQuery.ajax({
					url:'<?=base_url()?>get_bot_message.php',
					type:'post',
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-M873QDB"
